...
public class MainActivity extends Activity implements SopixUsb.Listener {
    ...
    private void buildUi(){
        ...
        TextView title=text("SOPIX T1",25,Color.WHITE);title.setTypeface(null,1);title.setGravity(Gravity.CENTER);root.addView(title,lp(-1,dp(42)));
        TextView subtitle=text("Dental Sensor",12,Color.rgb(145,164,184));subtitle.setGravity(Gravity.CENTER);root.addView(subtitle,lp(-1,dp(26)));
        ...
        mainButton=button("تهيئة الحساس والتقاط الصورة",Color.rgb(18,151,121));mainButton.setEnabled(false);mainButton.setOnClickListener(v->capture());LinearLayout.LayoutParams mainLp=lp(-1,dp(62));mainLp.setMargins(0,dp(14),0,dp(10));root.addView(mainButton,mainLp);

        LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.HORIZONTAL);row.setWeightSum(2);
        connectButton=button("إعادة الاتصال",Color.rgb(42,75,108));connectButton.setOnClickListener(v->usb.requestOrOpen());row.addView(connectButton,new LinearLayout.LayoutParams(0,dp(50),1));
    ...
    public void connected(boolean v){ui.post(()->{connected=v;mainButton.setEnabled(v);statusTitle.setText(v?"الحساس متصل وجاهز":"الحساس غير متصل");statusTitle.setTextColor(v?Color.rgb(80,225,176):Color.WHITE);statusSub.setText(v?"اكتب اسم المريض ثم اضغط زر الالتقاط":"صِل الحساس عبر USB OTG ثم أعد الاتصال");});}
